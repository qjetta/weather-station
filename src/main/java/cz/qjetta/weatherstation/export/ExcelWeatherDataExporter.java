...
public class ExcelWeatherDataExporter implements IDataExporter {
	...
	private ByteArrayInputStream convertWorkbookToInputStream(Workbook workbook)
			throws IOException {
	...
	private void createDataRows(List<WeatherDataDto> data, Sheet sheet) {
		...
		for (WeatherDataDto weather : data) {
			...
			row.createCell(3).setCellValue(weather.getWind());
			row.createCell(4).setCellValue(weather.getPrecipitation());
		}
	}

	private void createHeader(Sheet sheet) {
		Row headerRow = sheet.createRow(0);
		String[] columns = { "Timestamp", "Temperature", "Humidity", "Wind",
				"Precipitation" };
